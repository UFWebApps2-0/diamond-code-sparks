...
export default function AddStudents({ classroomId, addStudentsToTable }) {
  ...
  const handleManualAdd = async () => {
    ...
    if (res.data) {
      ...
      message.success(
        `${formattedName} has been added to the classroom groups successfully.`
      )
  ...
  const handleCsvAdd = async () => {
    ...
    if (res.data) {
      addStudentsToTable(res.data)
      message.success("added to classroom group successfully.")
    } else {
      message.error(res.err)
    }
  }
    ...
    <div id="add-students">
      <div id="manual-input">
        ...
        <form>
          ...
          <br />
          <input type="button" value="create group" onClick={handleManualAdd} />
        </form>
      </div>
      <Divider />
      <div>
        ...
        <p>
          CSV should have the following columns: "Groups" or "Student", (optional)
          "Animal"
        </p>
        <p>
